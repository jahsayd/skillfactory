...
from news.tasks import send_mail_for_sub_weekly
...
def news_sender():
    ...
    for category in Category.objects.all():
        ...
        # Третий цикл - для формирование письма
        # подписчикам, которые подписаны на категориею,
        # создаем приветственное письмо с нашим списком новых за неделю статей конкретной категории,
        # помещаем в письмо html шаблон , передаем в шаблон переменные и вызываем таск с рассылкой
        for subscriber in subscribers:
            ...
            send_mail_for_sub_weekly(sub_uname, sub_email, html_content)
